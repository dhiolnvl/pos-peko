-- Fix: duplicate invoice_number ketika 2 kasir transaksi bersamaan
--
-- Root cause: COUNT(*) tidak atomic — dua sesi bisa membaca jumlah yang sama
-- sebelum salah satu INSERT commit, sehingga keduanya generate invoice_number
-- yang identik dan salah satu gagal karena UNIQUE constraint.
--
-- Fix: ganti COUNT + langsung INSERT menjadi loop retry.
-- Setiap iterasi coba INSERT; kalau kena unique_violation (23505) increment
-- offset lalu coba lagi. Loop maksimal 10x (praktis tidak akan tembus).
...
BEGIN
  ...
  -- Hitung jumlah transaksi hari ini untuk branch ini sebagai base counter
  SELECT COUNT(*)
    INTO v_tx_count
    FROM transactions
  ...
  -- Loop retry: kalau invoice_number sudah dipakai (race condition), increment dan coba lagi
