...
import {
  ...
  TextInput, ActivityIndicator, RefreshControl, useWindowDimensions, Modal,
} from 'react-native';
...
export default function BackofficeTransactionsScreen() {
  ...
  // temp state untuk modal filter sebelum apply
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      let q = supabase
        .from('transactions')
        .select('id, invoice_number, total, payment_method, status, created_at, users!cashier_id(name), members!member_id(name)')
      ...
      setRows((data ?? []).map((r: any) => ({
        ...
        cashier_name: r.users?.name ?? '',
        member_name: r.members?.name ?? null,
      })));
    } catch (e) {
      console.error('[BOTransactions] load error:', e);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  }, [branchId, dateFilter, statusFilter, methodFilter]);
  ...
  return (
    <View style={styles.container}>
      <BackofficeHeader
        title="Transaksi"
        subtitle={DATE_LABELS[dateFilter]}
      />
      ...
      <View style={styles.searchRow}>
        <View style={styles.searchWrap}>
          ...
          <TextInput
            ...
            placeholder="Cari invoice, kasir, member..."
        ...
        <TouchableOpacity
          ...
          onPress={() => { setTempDate(dateFilter); setTempStatus(statusFilter); setTempMethod(methodFilter); setShowFilter(true); }}
        >
          ...
          <TouchableOpacity style={styles.filterModal} activeOpacity={1} onPress={() => {}}>
            ...
            <View style={styles.filterModalActions}>
              ...
              <TouchableOpacity
                ...
                onPress={() => { setDateFilter(tempDate); setStatusFilter(tempStatus); setMethodFilter(tempMethod); setShowFilter(false); }}
              >
      ...
      {loading && !refreshing ? (
        ...
        <FlatList
          ...
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); load(); }} tintColor="#56B2C1" />}
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              onPress={() => router.push(`/(backoffice)/transactions/${item.id}` as any)}
              activeOpacity={0.7}
            >
...
const styles = StyleSheet.create({
  ...
  searchInput: {
    flex: 1, paddingVertical: 12, paddingRight: 4, fontSize: 14, color: '#111827',
  },
  filterToggleBtn: {
