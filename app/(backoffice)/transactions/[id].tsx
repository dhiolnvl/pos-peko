  ...
  const loadDetail = useCallback(async () => {
    ...
    try {
      const { data: row } = await supabase
        .from('transactions')
        .select('id, invoice_number, subtotal, discount_amount, tax_amount, total, payment_method, payment_amount, change_amount, split_payment_detail, delivery_fee, notes, status, created_at, points_earned, users!cashier_id(name), branches!branch_id(name, address, phone), members!member_id(name, points)')
        .eq('id', id)
        .single();
