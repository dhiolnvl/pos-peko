    ...
    try {
      const { data: txData, error: txErr } = await supabase
        ...
        .select(`
          ...
          total, payment_method, payment_amount, change_amount, split_payment_detail, delivery_fee, notes,
          status, created_at, points_earned,
          users!cashier_id(name),
          branches!branch_id(name, address, phone),
          members!member_id(name, points)
        `)
        .eq('id', id)
        .single();

      if (txErr || !txData) {
        console.error('[TxDetail] query error:', txErr?.message, txErr?.details, txErr?.hint);
        return;
      }

      const row = txData as any;
      setTx({
        id: row.id,
        ...
        split_payment_detail: row.split_payment_detail ?? null,
        delivery_fee: row.delivery_fee ?? 0,
        notes: row.notes,
        status: row.status,
        created_at: row.created_at,
        points_earned: row.points_earned,
        cashier_name: row.users?.name ?? '-',
        branch_name: row.branches?.name ?? '-',
        branch_address: row.branches?.address ?? null,
        branch_phone: row.branches?.phone ?? null,
        member_name: row.members?.name ?? null,
        member_total_points: row.members?.points ?? null,
      });
  ...
  const handlePrint = async () => {
    ...
    await printReceipt({
      ...
      items: items.map((i) => ({
        ...
        discountAmount: i.discount_amount, subtotal: i.subtotal,
      })),
    ...
    try {
      ...
      for (const item of items) {
        ...
        const { data: bp } = await supabase
          ...
          .select('stock')
          .eq('product_id', txItem.product_id)
        ...
        await supabase
          .from('branch_products')
          .upsert(
            { branch_id: branchId, product_id: txItem.product_id, stock: newStock, updated_at: now },
      ...
      Alert.alert('Gagal', e.message ?? 'Terjadi kesalahan saat membatalkan transaksi.');
    } finally {
      setVoiding(false);
    }
  };
  ...
  if (!tx) {
    return (
      <View style={[styles.container, { paddingTop: insets.top, justifyContent: 'center', alignItems: 'center' }]}>
        ...
        <Text style={{ fontSize: 14, color: '#9CA3AF', marginTop: 8 }}>Transaksi tidak ditemukan</Text>
      </View>
    );
  }
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <ScrollView contentContainerStyle={[styles.scroll, { paddingBottom: insets.bottom + 24 }]}>
          ...
          <View style={styles.memberCard}>
            <View style={styles.memberCardLeft}>
              <View style={styles.memberAvatar}>
                <Text style={styles.memberAvatarText}>{tx.member_name.charAt(0).toUpperCase()}</Text>
              </View>
              <View>
            ...
            {(tx.points_earned ?? 0) > 0 && (
              <View style={styles.pointsBadge}>
                ...
                <Text style={styles.pointsBadgeText}>+{tx.points_earned} poin</Text>
              </View>
            )}
          </View>
        )}
        ...
        {canVoid && (
          <TouchableOpacity style={styles.voidBtn} onPress={() => setVoidModalVisible(true)}>
...
const styles = StyleSheet.create({
  ...
  refundBtn: {
    ...
    borderWidth: 1.5, borderColor: '#D97706', backgroundColor: '#FFFBEB', marginTop: 4,
  },
  ...
  voidBtn: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    gap: 8, paddingVertical: 14, borderRadius: 12,
    borderWidth: 1.5, borderColor: '#DC2626', backgroundColor: '#FEF2F2', marginTop: 4,
  },
