/**
 * Laporan Distribusi - Staff Pusat
...
function TransferRow({ transfer, isTablet }: { transfer: StockTransferHeader; isTablet: boolean }) {
...
export default function StaffPusatTransfersReport() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      // Ambil transfer dari semua gudang yang ada
      const results = await Promise.all(
        warehouses.map((w) => getStockTransfers(w.id))
      );
      const merged: StockTransferHeader[] = results.flat();
      setAllTransfers(merged);
    } catch (e: any) {
  ...
  // Filter hanya yang sudah dikirim dan dalam periode
                  ...
                  <View style={[styles.list, isTablet && styles.listTablet]}>
                    {filtered.map((t) => (
                      <View key={t.id} style={isTablet ? styles.listItemTablet : undefined}>
                        <TransferRow transfer={t} isTablet={isTablet} />
                      </View>
                    ))}
                  </View>
                )}
