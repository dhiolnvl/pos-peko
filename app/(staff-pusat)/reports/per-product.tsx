...
export default function PerProductReport() {
  ...
  useEffect(() => {
    fetchAllBranches().then((b) => setBranches(b ?? [])).catch(() => {});
  }, []);
  ...
  return (
    <View style={styles.container}>
      <OwnerPageHeader
        ...
        subtitle="Penjualan, HPP, dan laba kotor per produk"
        onBack={() => router.back()}
        rightElement={
          <TouchableOpacity
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: insets.bottom + 24 }}>
        ...
        <View style={{ height: 44, justifyContent: 'center' }}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.branchRow}>
            ...
            >
              <Text style={[styles.branchChipText, !selectedBranch && styles.branchChipTextActive]}>Semua</Text>
            </TouchableOpacity>
            {branches.map((b) => (
              <TouchableOpacity
                key={b.id}
...
const styles = StyleSheet.create({
  ...
  branchChipActive: { backgroundColor: '#EEF8FA', borderColor: '#347385' },
  branchChipText: { fontSize: 12, fontWeight: '600', color: '#6B7280' },
