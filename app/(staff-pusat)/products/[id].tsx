...
export default function ProductDetailScreen() {
  ...
  const navigateEvent = (e: StockEvent) => {
    if (e.type === 'po') router.push(`/(staff-pusat)/purchase/${e.targetId}?fromProduct=${id}` as any);
    else if (e.type === 'transfer') router.push(`/(staff-pusat)/transfers/${e.targetId}?fromProduct=${id}` as any);
    else router.push(`/(staff-pusat)/transactions/${e.targetId}?fromProduct=${id}` as any);
  };
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity
          ...
          onPress={() => router.push(`/(staff-pusat)/products/form?id=${id}` as any)}
        >
              ...
              recentTx.map((r: any, i) => {
                ...
                return (
                  <TouchableOpacity
                    key={i}
                    style={styles.txCard}
                    onPress={() => router.push(`/(staff-pusat)/transactions/${r.transaction_id}?fromProduct=${id}` as any)}
                    activeOpacity={0.75}
                  >
