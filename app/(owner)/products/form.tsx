...
export default function OwnerProductFormScreen() {
  ...
  useEffect(() => {
    ...
    if (!isEdit) setTimeout(() => setShowScanner(true), 300);
  }, []);
    ...
    try {
      ...
      if (selectedArr.length > 0) {
        const upsertRows = selectedArr.map((bId) => ({
          ...
          stock: parseInt(branchStock[bId] ?? '0') || 0,
              ...
              {branches.map((b) => {
                ...
                return (
                  <View key={b.id} style={styles.branchRow}>
                    <TouchableOpacity style={styles.branchCheckRow} onPress={() => toggleBranch(b.id)} disabled={isLoading}>
          ...
          <View style={styles.duplicateModal}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                router.back();
                setTimeout(() => router.push(`/(owner)/products/form?id=${dupId}`), 50);
              }}
            >
