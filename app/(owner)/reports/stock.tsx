...
function BranchCard({
  summary, color, isSelected, onPress, flex,
}: {
  ...
  return (
    <TouchableOpacity
      style={[bStyles.card, flex && { flex: 1, width: undefined }, isSelected && { borderColor: color, backgroundColor: color + '08' }]}
      onPress={onPress}
      activeOpacity={0.75}
    >
...
export default function StockReport() {
  ...
  const load = useCallback(async () => {
    ...
    setCrossStock([]);
    try {
      const allBranches = await fetchAllBranches() as OwnerBranch[];
  ...
  // Potong data yang ditampilkan sesuai halaman yang sudah di-load
  const visibleData = useMemo(() => filtered.slice(0, visibleCount), [filtered, visibleCount]);

  // Tablet: groupkan per baris 2 kolom untuk render
  ...
  const listHeader = useMemo(() => (
    ...
    stockFilter, search, filtered.length, visibleCount, branches, isTablet,
    toggleBranch,
  ]);
...
const styles = StyleSheet.create({
  ...
  // Header — satu layout untuk semua ukuran layar
  header: {
