...
const FILTER_OPTIONS: {
  ...
  { key: "owner", label: "Owner", icon: "shield-checkmark", color: "#347385" },
  {
    key: "staff_pusat",
    label: "Staff Pusat",
    icon: "briefcase",
  ...
  if (compact) {
    return (
      <View style={statStyles.cardCompact}>
        <View
          style={[statStyles.iconBoxCompact, { backgroundColor: color + "15" }]}
        >
          <Ionicons name={icon} size={14} color={color} />
        </View>
        <Text style={[statStyles.valueCompact, { color }]}>{value}</Text>
        <Text style={statStyles.labelCompact} numberOfLines={1}>
          {label}
        </Text>
      </View>
    );
...
function UserCard({
  ...
  return (
    <View
      style={[
        cardStyles.card,
        { borderColor: rc.dot + "30" },
        !user.is_active && cardStyles.cardInactive,
      ]}
    >
      <View style={cardStyles.body}>
        ...
        <View style={cardStyles.topRow}>
          <View style={[cardStyles.avatarRing, { borderColor: rc.dot }]}>
            <View style={[cardStyles.avatar, { backgroundColor: rc.bg }]}>
              <Text style={[cardStyles.avatarText, { color: rc.text }]}>
                {initial}
              </Text>
            </View>
          </View>
          ...
          <View style={[cardStyles.roleBadge, { backgroundColor: rc.bg }]}>
            <Ionicons name={ROLE_ICONS[user.role]} size={12} color={rc.text} />
        ...
        {user.branch && (
          <View style={cardStyles.branchRow}>
            ...
            <Text style={cardStyles.branchText} numberOfLines={1}>
              {user.branch.name}
            </Text>
          </View>
        )}
        ...
        <View style={cardStyles.actions}>
          <TouchableOpacity style={cardStyles.actionBtn} onPress={onEdit}>
            ...
            <Text style={[cardStyles.actionText, { color: "#347385" }]}>
              Edit
            </Text>
          </TouchableOpacity>
          ...
          >
            ...
            <Text style={[cardStyles.actionText, { color: "#B45309" }]}>
              Reset Pass
            </Text>
          </TouchableOpacity>
          <View style={cardStyles.actionSep} />
...
function UserForm({ user, branches, onClose, onSaved }: UserFormProps) {
  ...
  const availableRoles: Role[] = isEdit
    ? ["owner", "back_office", "cashier"]
    : ["back_office", "cashier"];

  const [role, setRole] = useState<Role>(
    user?.role &&
      (["owner", "back_office", "cashier"] as Role[]).includes(
        user.role as Role,
      )
      ? (user.role as Role)
      : "cashier",
                ...
                {availableRoles.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <View
                        style={[
                          fmStyles.roleIconBox,
                          active && { backgroundColor: rc.dot + "22" },
                        ]}
                      >
                        <Ionicons
                          name={ROLE_ICONS[r]}
                      ...
                      <Text
                        style={[
                          fmStyles.roleLabel,
                          active && { color: rc.text, fontWeight: "700" },
                        ]}
                      >
                        {ROLE_LABELS[r]}
                      </Text>
                ...
                {branches.map((b, index) => {
                  ...
                  return (
                    <React.Fragment key={b.id}>
                      ...
                      <TouchableOpacity
                        style={fmStyles.branchRow}
                        onPress={() => setBranchId(b.id)}
                      >
                        <View
                          style={[
                            fmStyles.branchDot,
                            { backgroundColor: active ? "#56B2C1" : "#D1D5DB" },
                          ]}
                        />
                        <Text
                          style={[
                            fmStyles.branchName,
                            active && fmStyles.branchNameActive,
                          ]}
                        >
                          {b.name}
                        </Text>
...
function ResetPasswordModal({
  ...
  const save = async () => {
    ...
    setError("");
    try {
      const { error: e } = await supabase.functions.invoke(
        "reset-user-password",
        {
          ...
          <View style={[rpStyles.sheet, { paddingBottom: insets.bottom + 24 }]}>
            ...
            {done ? (
              ...
              >
                ...
                <Text style={rpStyles.successTitle}>
                  Password Berhasil Direset
                </Text>
                <Text style={rpStyles.successSub}>
                  Beritahu pengguna password barunya
                </Text>
                <TouchableOpacity
                  style={{
                    alignSelf: 'stretch',
              ...
              <>
                ...
                <View style={{ flexDirection: "row", gap: 10 }}>
                  <TouchableOpacity
                    style={rpStyles.btnSecondary}
                    onPress={onClose}
                  >
                    <Text style={rpStyles.btnSecondaryText}>Batal</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={rpStyles.btnPrimary}
                    onPress={save}
                    disabled={saving}
                  >
                    {saving ? (
...
export default function UsersManagement() {
  ...
  const [formUser, setFormUser] = useState<Partial<UserRow> | null | false>(
    false,
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      branchesData.forEach((b) => {
        branchMap[b.id] = { id: b.id, name: b.name };
      });
  ...
  // Tablet: 2-column grid, phone: 1 column
        ...
        rightElement={
          <View style={{ flexDirection: "row", gap: 6 }}>
            <TouchableOpacity
              ...
              onPress={() => router.push("/(owner)/management/branches" as any)}
            >
        ...
        <View style={[styles.contentInner, isTablet && { maxWidth: 960 }]}>
          {loading ? (
            ...
            <FlatList
              ...
              numColumns={numCols}
              columnWrapperStyle={
                numCols > 1 ? styles.columnWrapper : undefined
              }
                            ...
                            {active && (
                              ...
                              >
                                <Text style={styles.chipBadgeText}>
                                  {item.key === "all"
                                    ? users.length
                                    : users.filter((u) => u.role === item.key)
                                        .length}
                                </Text>
                              </View>
                            )}
                ...
                <View style={styles.centered}>
                  ...
                  <Text style={styles.emptySub}>
                    {search
                      ...
                      : "Tambahkan user pertama dengan tombol +"}
                  </Text>
                </View>
              }
